(function () {
  ...
  const state = {
    ...
    quickEditing: false, quickEditWasPaused: true
  };
  ...
  function isDue(card) { if (isNew(card)) return false; if (!card?.nextReviewAt) return true; const d = new Date(card.nextReviewAt); return Number.isNaN(d.getTime()) || d <= endToday(); }
  ...
  function showBack(show){state.revealed=show;$("#multiBackSection").classList.toggle("hidden",!show);$("#multiRatings").classList.toggle("hidden",!show);$("#multiEditArea").classList.toggle("hidden",!show);$("#multiHint").classList.toggle("hidden",show);}
  ...
  function render(){const e=current();if(!e){finish();return;}$("#multiFront").innerHTML=e.card.frontHtml||"";$("#multiBack").innerHTML=e.card.backHtml||"";$("#multiDeckChip").textContent=state.decks.get(e.card.deckId)?.name||"Baralho";updateControls();showBack(false);updateProgress();$("#multiPrev").disabled=state.index<=0;$("#multiNextArrow").disabled=state.index>=state.queue.length-1;$("#multiCard").focus({preventScroll:true});}
  ...
  async function resetProgress(cards){const patch={reviewStatus:null,lastRating:null,lastReviewedAt:null,reviewCount:0,currentIntervalDays:null,nextReviewAt:null,ratingHistory:[]};for(const c of cards){await OituDB.updateCard(c.id,patch);Object.assign(c,patch);}}
  ...
  async function openConfig(deckIds,label="Estudar selecionados"){
    ensureUI();stopTimer();state.active=false;state.deckIds=[...new Set(deckIds)].filter(Boolean);state.label=label;state.decks=new Map();state.cards=[];
    ...
    const loaded=await Promise.all(state.deckIds.map(async(id)=>{const [deck,cards]=await Promise.all([OituDB.getDeck(id),OituDB.getCardsByDeck(id)]);return deck?{deck,cards:cards||[]}:null;}));
    loaded.forEach((item)=>{if(!item)return;state.decks.set(item.deck.id,item.deck);state.cards.push(...item.cards);});
