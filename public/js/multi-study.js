(function () {
  ...
  async function openConfig(deckIds,label="Estudar selecionados"){
    ...
    for(const id of state.deckIds){const deck=await OituDB.getDeck(id);if(!deck)continue;state.decks.set(id,deck);const cards=await OituDB.getCardsByDeck(id);state.cards.push(...cards);}
