(function () {
  ...
  function preloadNext(){const active=current()?.card;if(active)window.OituMedia?.prefetch?.(active.backHtml||"");const upcoming=state.queue.slice(state.index+1,state.index+3).map(entry=>entry?.card).filter(Boolean);if(!upcoming.length)return;const run=async()=>{await Promise.allSettled(upcoming.map(async card=>{await ensureCardLoaded(card);await window.OituMedia?.prefetch?.(card.frontHtml||"",card.backHtml||"");}));};setTimeout(run,0);}
